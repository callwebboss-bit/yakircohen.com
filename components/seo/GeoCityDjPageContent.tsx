...
function buildFaqItems(cityNamePrep: string): FAQItem[] {
  return [
    ...
    {
      id: "travel",
      question: `יש תוספת הגעה ${cityNamePrep}?`,
      answer:
...
export type GeoCityDjPageContentProps = {
  citySlug: GeoCitySlug;
};

export default function GeoCityDjPageContent({
  citySlug,
}: GeoCityDjPageContentProps) {
  const city = getGeoCity(citySlug);
  const pagePath = city.djPath;
  const pathname = `/${pagePath}`;
  const formId = `dj-${citySlug}-form`;
  const faqItems = buildFaqItems(city.nameHePrep);
    ...
    "@graph": [
      {
        ...
        "@id": `${SITE_URL}/${pagePath}#dj-service`,
  ...
  const whatsappHref = buildWhatsAppHref({
    text: city.whatsappDj,
    utm_source: "website",
    utm_campaign: city.utm.dj,
  });
  ...
  return (
    <article className="bg-background">
      ...
      <SectionDwellTracker sectionId={`dj-${citySlug}-boutique`} eventLabel="boutique" />
      <SectionDwellTracker sectionId={`dj-${citySlug}-academy`} eventLabel="academy" />
      ...
      <section
        ...
        aria-labelledby={`dj-${citySlug}-audience-heading`}
      >
        <FadeIn className="mx-auto max-w-3xl">
          <h2
            id={`dj-${citySlug}-audience-heading`}
            className="text-2xl font-semibold tracking-tight text-foreground sm:text-3xl"
          >
            הקהל שלכם, השפה שלנו
          </h2>
          <p className="mt-6 text-base leading-relaxed text-muted-foreground sm:text-lg">
            {city.djAudienceIntro}
          </p>
          {city.djReligiousAudience ? (
            <ul className="mt-12 grid gap-5">
              {RELIGIOUS_AUDIENCE_POINTS.map((point) => (
                <li
                  key={point.title}
      ...
      <section
        className="border-t border-border px-4 py-10 sm:px-6 sm:py-20"
        aria-labelledby={`dj-${citySlug}-method-heading`}
      >
        <FadeIn className="mx-auto max-w-3xl">
          <h2
            id={`dj-${citySlug}-method-heading`}
            className="text-2xl font-semibold tracking-tight text-foreground sm:text-3xl"
          >
      ...
      <section
        id={`dj-${citySlug}-boutique`}
        className="border-t border-border px-4 py-10 sm:px-6 sm:py-20"
        aria-labelledby={`dj-${citySlug}-boutique-heading`}
      >
        <FadeIn className="mx-auto max-w-3xl">
          <h2
            id={`dj-${citySlug}-boutique-heading`}
            className="text-2xl font-semibold tracking-tight text-foreground sm:text-3xl"
          >
            יקיר כהן - רמת בוטיק
          </h2>
      ...
      <section
        id={`dj-${citySlug}-academy`}
        className="border-t border-border px-4 py-10 sm:px-6 sm:py-20"
        aria-labelledby={`dj-${citySlug}-academy-heading`}
      >
        <FadeIn className="mx-auto max-w-3xl">
          <h2
            id={`dj-${citySlug}-academy-heading`}
            className="text-2xl font-semibold tracking-tight text-foreground sm:text-3xl"
          >
            האקדמיה - חלופה חכמה
          </h2>
          ...
          <div id={formId} className="mt-12 scroll-mt-24">
            <CallbackLeadForm
      ...
      <section
        className="border-t border-border px-4 py-10 sm:px-6 sm:py-20"
        aria-labelledby={`dj-${citySlug}-production-heading`}
      >
        <FadeIn className="mx-auto max-w-5xl">
          <h2
            id={`dj-${citySlug}-production-heading`}
            className="text-2xl font-semibold tracking-tight text-foreground sm:text-3xl"
          >
      ...
      <section
        className="border-t border-border px-4 py-10 sm:px-6 sm:py-20"
        aria-labelledby={`dj-${citySlug}-cta-heading`}
      >
        <FadeIn className="mx-auto max-w-3xl text-center">
          <h2 id={`dj-${citySlug}-cta-heading`} className="sr-only">
            יצירת קשר
          </h2>
      ...
      <div className="border-t border-border px-4 py-10 sm:px-6 sm:py-16 lg:px-8">
        <div className="mx-auto max-w-3xl space-y-12">
          <FAQAccordion
            title={`שאלות נפוצות - DJ ${city.nameHePrep}`}
            items={faqItems}
          />

          <section aria-labelledby={`dj-${citySlug}-aeo-heading`}>
            <h2
              id={`dj-${citySlug}-aeo-heading`}
            ...
            <div className="mt-6 space-y-6">
              <div>
                <h3 className="font-semibold text-foreground">
                  {`מה כולל מפרט הציוד הבסיסי לאירוע ${city.nameHePrep}?`}
                </h3>
              ...
              <div>
                <h3 className="font-semibold text-foreground">
                  {`האם יש תוספת מחיר על הגעה ${city.nameHePrep}?`}
                </h3>
                <p className="mt-1 text-sm text-muted-foreground">
          ...
          <PageRelatedFooter pathname={pathname} />
        </div>
      </div>
    </article>
  );
